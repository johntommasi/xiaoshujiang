...
Pax6实验粉碎了这个理解。它证明了一件事：虽然最终的形态截然不同，但驱动眼睛发育的基因调控网络(gene regulatory network
- GRN)在5亿年里被保存了下来。昆虫和哺乳动物用的是同一套上游指令集，只是下游的执行模块不同。
...
范畴论中最深刻的定理之一叫米田引理(Yoneda lemma)。它说的是：一个对象完全由它与所有其他对象的关系决定。更精确地说，对于范畴C中的任意对象X，X的"本质"可以被完全捕捉为一个函子Hom(-
- X)，即"所有其他对象到X的态射的集合"。两个对象如果跟所有其他对象的关系模式完全相同，那它们就是同构的(本质上相同的)。
