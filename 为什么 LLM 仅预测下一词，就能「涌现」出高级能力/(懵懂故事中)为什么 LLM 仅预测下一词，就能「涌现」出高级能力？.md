...
| 比较维度      | 传统复杂系统中的涌现                                                                                           | 早期语言模型 (LLM之前)                                                                      | LLM涌现初现阶段 (GPT-3)                                                                                                                        | 当前最先进LLM                                                                                                                                     |
| --------- | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 定义及理论基础   | 源于复杂性科学：“整体行为无法由单一部分属性解释”的现象；存在弱/强涌现之分（弱涌现指整体行为难预测但不违背微观规律，强涌现指整体出现全新不可还原的性质）。                       | 无专门“涌现”概念，属模块化设计的统计模型；模型行为可由局部概率规则解释。性能随规模平滑提升，缺乏质变跳跃。                              | 借用复杂系统概念描述大模型的新能力：小模型不具备的能力在大模型中突然出现，无法从小模型性能线性外推预测。概念提出源于GPT-3等模型的实验观察                                                                  | 延续LLM涌现概念并走向“通用智能”讨论：超大模型展现跨领域综合智能，被视为可能出现强涌现征兆  <br>。理论上涉及多模态融合、大规模训练及强化学习等新元素带来的综合效应。                                                      |
| 整体 > 部分之和 | 是典型特征：整体涌现出各部分单独不具备的新属性，整体功能超越部分简单相加。如蚁群智能、生命从化学中涌现等，体现出强协同效应。                                       | 不明显：模型整体功能几乎是各部件功能的加总，没有产生额外的“神秘”性质。模型按照预设机制工作，整体可分解为局部统计贡献之和。                      | 明显具有：庞大参数间出现协同作用，模型表现出超过训练语料简单记忆的综合能力。GPT-3 显示的上下文学习等能力并非任何单个神经元所独有，而是整体网络共同涌现的结果。                                                       | 更加强烈：模型规模和多模态融合使涌现协同效应加剧。GPT-4可将图像+文本结合理解复杂语义，这是单一模态模型无法实现的整体新能力  <br>。整体系统（模型+对齐机制）表现出超出各部分功能之和的智能水平。                                       |
| 不可预测性     | 高：宏观行为难以从微观定律直接预测，需依赖模拟或经验观察  <br>。复杂系统常出现出乎意料的模式（例如混沌、临界相变），属于涌现的神秘之处。                              | 低：模型行为基本可预测，性能提升随参数/数据增加呈渐进趋势。没有突然冒出的新技能，小模型的表现可用于推估大模型的表现。                         | 高：出现不可预测的能力跳变。许多任务上，大模型突然远胜小模型；这些能力在训练前未被预计。无法仅凭小模型推断出GPT-3的那些新本领。                                                                       | 高：仍有显著不可预测性，甚至模型创造者也难完全预料其行为  <br>。随着复杂度提高，模型可能涌现新的（有时是潜在危险的）能力和偏差  <br>。研究者持续通过评测发现GPT-4/Claude的新能力和盲点。                                     |
| 反馈机制      | 存在反馈回路：宏观涌现结果影响微观个体（上下互动）。如价格作为宏观量反馈调控个体行为，形成循环因果。反馈机制常使系统达到自组织稳定或进一步演化。                             | 无内生反馈：模型为前馈结构，输出不改变模型内部状态。运行过程中没有宏观行为反过来影响模型参数的机制，一次生成过程互相独立。                       | 基本无反馈（推理时）：Transformer生成属于前馈自回归；输出作为新输入上下文是序列依赖，但模型参数不因自身输出更新。训练阶段亦未采用环境反馈（纯预测训练）。                                                       | 引入外部反馈（训练阶段）：通过人类/AI反馈微调模型。GPT-4 使用人类偏好反馈强化学习对齐；Claude 2 采用“宪法AI”让模型自我批改输出，利用AI自身评价作为奖励信号。推理阶段模型本身仍无参数更新反馈，但多轮对话中模型能参考自己先前输出（上下文）进行调整。       |
| 验证方式      | 观察和实验并重：通过观测宏观模式出现来验证涌现（如相变实验、群体行为观测）；通过模型模拟再现涌现现象（如生命游戏模拟蚁群等） <br>。近期发展出定量指标（有效信息、协同信息等）来检测和测量涌现程度。 | 常规评测：无特殊涌现能力需验证，主要用性能指标评估模型效果，如困惑度随模型规模下降等  <br>。验证关注模型是否按预期改进，无“突然出现的新行为”需要额外验证手段。 | 设计专项基准测试验证：比较不同参数规模模型在新颖任务上的表现，寻找性能非线性跃升点  <br>。例如BIG-bench等测试显示GPT-3级别模型在某些任务上准确率从随机水平跳升，证实了涌现能力存在。研究通过控制变量实验确保这些能力并非数据泄漏而确为规模带来的新泛化行为。 | 严格全面评测：通过标准化考试和挑战任务验证模型新能力边界。例如GPT-4参加专业考试和复杂推理测试，表现接近人类证明其综合能力飞跃。同时进行安全测试评估是否涌现出潜在风险能力。研究者也尝试用更细粒度测度使能力随规模变化曲线平滑，以验证“突现”是否可以被解释为指标选择问题。<br> |
...
-    **当前最先进 LLM 阶段（GPT-4、5 等）：最新一代的超大规模模型（例如GPT-4、Google DeepMind Gemini、Claude 2等）延续并扩展了前述涌现概念。理论上，这些模型由于规模和训练方法的飞跃，被一些研究者视为在朝“通用智能”方向发展[arxiv.org](https://arxiv.org/abs/2303.12712#:~:text=exhibit%20more%20general%20intelligence%20than
- In%20our%20exploration)。GPT-4 等模型不仅在语言任务上表现卓越，还能跨模态和跨领域解决复杂问题，展现出更加广泛和深入的智能能力。微软研究人员称GPT-4展示出“通用智能的火花”，因为它无需特殊提示就能在数学、编程、医学、法律等前所未有的广泛任务上接近人类水平** （如下图
- _用几条“非常普通的指令”就让 GPT-4 同时完成诗歌化数学证明、代码生成图形（TikZ 独角兽）、Python 动画、以及数学推理题，体现的是一种跨任务/跨表征的“组合能力”突然变得稳定可用——这更像能力阈值被跨过后的质变，而不是单一指标的线性提升。_ ） **。总体而言，当前阶段的涌现行为被视作此前规模扩展趋势的延续和升级，并引发关于强涌现** （真正的新涌现规律）的讨论。
