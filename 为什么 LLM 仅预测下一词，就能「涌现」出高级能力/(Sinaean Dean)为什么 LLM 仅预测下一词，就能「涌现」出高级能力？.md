...
你知不知道LLM推理优化的一个手段叫`多头美杜莎`，这就是LLM脑子里已经有了下好几个字的证据。为防止别人不懂，我把图贴下面了。多头美杜莎就是在LLM的最后进行了修改，本来那里只有一个lmhead
- 这个lm head输出的是下一个token。多头美杜莎的修改就是最后有多个lm head，叫medusa head 1
- 2
- 3，每个分别预测下一个，下下个，下下下个token。
